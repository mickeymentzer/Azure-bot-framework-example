        ...
        public async Task<HttpResponseMessage> SendMessage()
        {
            try
            {
                if (!string.IsNullOrEmpty(ConversationStarter.fromId))
                {
                    ...
                    resp.Content = new StringContent($"<html><body><h1>Nice Work!</h1><a href=\"http://localhost:62334/api/CustomWebApi\">Click it again, you know you want to.</a></body></html>", System.Text.Encoding.UTF8, @"text/html");
